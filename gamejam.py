...
intro_text = f"""Welcome to the {colors.Green}interdimensional maze{colors.Reset}.
Here you can interact with all kinds of {colors.Magenta}creatures{colors.Reset} and {colors.Blue}items{colors.Reset}."""

tutorial_text = f"""
You can move with {colors.Cyan}w, a, s, d{colors.Reset}. You are represented by the {colors.Magenta};{colors.Reset} symbol in the maze.
"""
...
def print_map():
    ...
    if maze[player_y][player_x] != ";":
        prev_symbol = maze[player_y][player_x]
    maze[player_y][player_x] = ";"

    print(f"{colors.Green}[CURRENT MAZE ROOM: {maze_room}]{colors.Green}")
    for row in maze:
        print(colors.Magenta + "".join(row) + colors.Reset)
    print("")
...
def walk():
    ...
    if direction == 'w':
        #print("moved up")
        maze[player_y][player_x] = prev_symbol
        player_y -= 1
        ...
        if player_x >= len(maze[0]):
            player_x = len(maze[0]) - 1

def interactions():
    if player_y in ref and player_x in ref[player_y]:
        creature = ref[player_y][player_x]
        creature.appear()
        creature.interact()
        print(f"[You've interacted with {creature.name}].")
        print_map()
